...
public class DrawingPanel extends JPanel implements MouseListener, MouseMotionListener {
	...
	public void mousePressed (MouseEvent arg0) {} // Not used
	...
	public void mouseClicked (MouseEvent arg0) {
		...
		resetCurrentImage();
		if (status == "CIRCLER") {
			// This will ask for the radius of the Circle that is going to be drawn 
			...
			try {
				...
				radius = 1; //by default if it can't parse an integer, the radius
				//will be one.
			}
			generateCircle();
		}
		if (status == "LINER"){
			// This will ask for the amount of Lines that are going to be drawn after 
			// every time the user presses the mouse over the drawing area
			String res = JOptionPane.showInputDialog("Enter desired amount of Lines:");
			try {
				lines = Integer.parseInt(res);
			} 
			catch (Exception e) {
				lines = 0; //by default if it can't parse an integer, the amount of
				//lines will be 0.
			}
			generateRandomLines(); 
		}
	...
	private void doLines(Graphics2D g2d){
		g2d.setColor(new Color(192, 0, 192));
		paintDots(g2d);
	...
	private void doCursor(Graphics2D g2d){
		// A hovering sign with the current coordinates of the mouse, this one is
		// not drawn into the currentImage because the idea is to show the coordi-
		// nates of the cursor hovering the image, which instead of being re-ren-
	...
	private void paintDots(Graphics2D g2d) {
		// Draw in the screen the 500 random dots to draw Lines between
		// afterwards
	...
	private void bresenhamLine(int x1, int y1, int x2, int y2, Graphics2D g2d) {
		...
		//Algorithm requires dx and dy to be positive, having an absolute dx doesnt
		//matter from here on because it will start from the leftmost x until the
		//rightmost x (a positive direction), and having an absolute dy doesnt matter
		//because it will be only used for calculation, direction of change is set
		//individualy to be either positive or negative. 
		dy = Math.abs(dy);
		dx = Math.abs(dx);
		
		if (Math.abs(m) > 1) {
			// If the absolute value of the slope is greater than one, this will get
			// a new line where the absolute value is less than one, for the algorithm
			// to work. The original slope is preserved so when pixels are being drawn, 
			// if the original absolute value was greater than one, the coordinates are
			//swapped back at the time of printing.
		...
		if (x2 < x1) {
			// This is so we can use the absolute value of dx, since we are starting
			// always at the leftmost point, going in a positive direction to the one
			// that its at the rightmost.
		...
		// Whats REALLY fast about this algorithm (and precise) is that it only uses 
		// addition and comparissons, the only needed multiplications (which increases 
		// error) are computed only once. also, it only uses integers, and that gives 
		// a lot of precission and speed.
		while (dx > 0) {
			if (d <= 0) {
				d += incE;
			} else {
			...
			if (Math.abs(m) > 1) {
				drawPixel(g2d, y1, x1); // Swapped-back ordering
			} else {
				drawPixel(g2d, x1, y1); // Normal ordering
			}
			dx--;
			x1++;
		}
	...
	private void bresenhamCircle(int cx, int cy, int radius, Graphics2D g2d) {
		...
		while (x <= y) {
			...
			// x and y coordinates, and adding the current position of the mouse,
			// without changing that one!
			drawPixel(g2d, x + cx, y + cy);
			drawPixel(g2d, y + cx, x + cy);
			drawPixel(g2d, -x + cx, y + cy);
			drawPixel(g2d, -y + cx, x + cy);
			drawPixel(g2d, x + cx, -y + cy);
			drawPixel(g2d, y + cx, -x + cy);
	...
	public void paintComponent(Graphics g) {
		...
		if (curImage == null){
			// This is to avoid re-rendering each of the circles/lines when the screen
			// is refreshed (which happens due to the constantly showing of the current
			// mouse coordinates.
			curImage = new BufferedImage(dimX, dimY, BufferedImage.TYPE_INT_ARGB);
			curImageG2D = curImage.createGraphics();
			if (status == "LINER")    doLines(g2d);
			if (status == "CIRCLER") doCircles(g2d);
			doAxis(g2d);
		}
		else{
			// If the image is not null, it means nothing has made it null (like,
			// changing between modes, resetting a mode, inserting a new circle or
			// changing the desired amount of lines) so simply draw the image
