...
public class DrawingPanel extends JPanel implements MouseListener,
        MouseMotionListener {
    private BufferedImage curImage; // Current BG image to avoid re-drawing all
                                    // shapes
    ...
    private ArrayList<Integer> dotX; // List of random X coordinates for 500
                                        // points
    private ArrayList<Integer> dotY; // List of random Y coordinates for 500
                                        // points
    ...
    // When the status of the application is changed by a button from
    // OptionPanel
    ...
    public void mouseClicked(MouseEvent arg0) {
        ...
        if (status == "CIRCLER") {
            ...
            String res = JOptionPane
                    .showInputDialog("Enter radius of desired Circle");
            try {
        ...
        if (status == "LINER") {
            ...
            String res = JOptionPane
                    .showInputDialog("Enter desired amount of Lines:");
            try {
    ...
    private void doCursor(Graphics2D g2d) {
        ...
        // is
        // not drawn into the currentImage because the idea is to show the
        // coordi-
    ...
    private void bresenhamLine(int x1, int y1, int x2, int y2, Graphics2D g2d) {
        ...
        if (x1 == x2) {
            ...
            // so when swapping coordinates, it will be seen as a horizontal
            // line
        } else {
        ...
        // doesnt
        // matter from here on because it will start from the leftmost x until
        // the
        // rightmost x (a positive direction), and having an absolute dy doesnt
        // matter
        // because it will be only used for calculation, direction of change is
        // set
        ...
        if (Math.abs(m) > 1) {
            ...
            // get
            // a new line where the absolute value is less than one, for the
            // algorithm
            // to work. The original slope is preserved so when pixels are being
            // drawn,
            // if the original absolute value was greater than one, the
            // coordinates are
        ...
        if (x2 < x1) {
            ...
            // starting
            // always at the leftmost point, going in a positive direction to
            // the one
        ...
        // uses
        // addition and comparissons, the only needed multiplications (which
        // increases
        // error) are computed only once. also, it only uses integers, and that
        // gives
    ...
    public void paintComponent(Graphics g) {
        ...
        if (curImage == null) {
            ...
            // screen
            // is refreshed (which happens due to the constantly showing of the
            // current
            // mouse coordinates.
            ...
            // If the image is not null, it means nothing has made it null
            // (like,
